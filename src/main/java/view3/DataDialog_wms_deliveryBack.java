...
public class DataDialog_wms_deliveryBack extends JDialog {
	...
	public static void main(String[] args) {
		try {
			DataDialog_wms_deliveryBack dialog = new DataDialog_wms_deliveryBack();
	 ...
	 * Create the dialog.
	 */
	public DataDialog_wms_deliveryBack() {
		setTitle("填写订单号");
	...
	protected void relatedToData() {
		...
		SSHService.writeProperties("order_sn",textField_order_sn.getText()+"a");
...
//		AutoLogger.log.info("订单号："+SSHService.order_sn());
	...
	protected void save() {
		...
		if(ownerCode.trim().isEmpty() || order_sn.trim().isEmpty()) {
...
//			new RunDialog_delivery1().setVisible(true);
			new RunDialog_wms_deliveryBack().setVisible(true);

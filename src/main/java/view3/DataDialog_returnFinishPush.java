...
import view4.RunDialog_returnFinishPush;
...
public class DataDialog_returnFinishPush extends JDialog {
	...
	public static void main(String[] args) {
		try {
			DataDialog_returnFinishPush dialog = new DataDialog_returnFinishPush();
	 ...
	 * Create the dialog.
	 */
	public DataDialog_returnFinishPush() {
		setTitle("填写订单号");
...
//		AutoLogger.log.info("订单号："+SSHService.order_sn());
	...
	protected void save() {
		...
		if(deal_code.trim().isEmpty()) {
			...
			AutoLogger.log.info("平台订单号："+SSHService.deal_code());
			this.dispose();
			new RunDialog_returnFinishPush().setVisible(true);
